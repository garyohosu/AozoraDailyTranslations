...
### Q5-3 Publisher の部分失敗時の state.json 更新 `OPEN`

- SPEC.md §6.5 に「sitemap/robots.txt 生成が失敗しても作品ページが成功なら続行」とある。
- この場合、6A（作品ページ）は生成済みだが 6C（sitemap）は古いままでコミットされる。
  - `state.json` の `next_index` は更新してよいか（その作品は「公開済み」扱い）？
  - 次回実行で sitemap が自然に更新されるなら OK だが、「全公開ファイル + state.json + logs を同一コミット」という原則との整合性は？
...
### Q6-3 `aozora.gr.jp` のサブドメイン許可範囲 `OPEN`

- URL バリデーションで「`aozora.gr.jp` ドメインのみ許可」とあるが、実際の配信 URL は `www.aozora.gr.jp` のほか `cdn.aozora.gr.jp` 等のサブドメインが使われる可能性がある。
- 許可する FQDN の一覧（ホワイトリスト）を明示すべき。
...
### Q10-1 並行実行防止の具体的メカニズムが未定義 `OPEN`

- SPEC.md §6.3「openClaw must be configured to prevent concurrent runs」とあるが、具体的な実装が未定義。
- openClaw が GitHub Actions の場合は `concurrency` グループで制御可能。独自サービスの場合はロックファイル（`.lock`）等が必要。
- Q2-1 の回答よりローカルの openClaw を使用するため、openClaw 自身の単一実行保証機能を確認する。
...
### Q10-3 `state.json` の `rollback` ステータスの扱い `OPEN`

- USECASE.md §5 のステートマシンには `rollback` 状態が定義されているが、SPEC.md §3.1 の `state.json` スキーマには `active` と `exhausted` しか記載されていない。
- `rollback` は過渡的な状態（手動操作中のみ存在）として `state.json` の `status` フィールドに書き込むのか、それともドキュメント上の概念だけで実際のファイルには出現しないのか？
